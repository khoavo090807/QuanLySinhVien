...
------------------------------------------------------------
IF DB_ID('QLSV_DoAn') IS NOT NULL
BEGIN
    ALTER DATABASE QLSV_DoAn SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE QLSV_DoAn;
END
GO
...
PRINT N'Tạo CSDL QLSV_DoAn thành công!'
GO
...
* Mục đích: Xóa một môn học khỏi CSDL một cách an toàn.
* Logic:
	1. Kiểm tra xem môn học có đang được MỞ LỚP HỌC PHẦN không. Nếu có -> Dừng.
	2. Kiểm tra xem môn học có đang là MÔN TIÊN QUYẾT cho môn khác không. Nếu có -> Dừng.
	3. Nếu an toàn, tự động xóa môn này khỏi Bảng 11 (MonHoc_TienQuyet).
	4. Xóa môn học khỏi Bảng 7 (MonHoc).
------------------------------------------------------------
*/
...
END
GO
-- Thử xóa môn "Mạng máy tính" (MMT)
-- Môn này sẽ XÓA THÀNH CÔNG (vì nó mạng máy tính ko là môn học tuyên quyết)
select * from MonHoc_TienQuyet
select * from MonHoc
EXEC sp_XoaMonHoc @MaMH = N'MMT';
select * from MonHoc
-- Thử xóa môn "Hướng đối tượng" (HDT)
-- Môn này sẽ BÁO LỖI (vì nó là tiên quyết của LTW)
select * from MonHoc_TienQuyet
EXEC sp_XoaMonHoc @MaMH = N'HDT';
GO

-- Thử xóa môn "Cơ sở dữ liệu" (CSDL)
-- Môn này sẽ BÁO LỖI (vì đã được mở lớp LHP01, LHP08)
select * from LopHocPhan
order by MaLHP
EXEC sp_XoaMonHoc @MaMH = N'CSDL';
GO
...
-- TRUY VẤN 1: Chi tiết sinh viên đăng ký, MÔN HỌC và HỌC KỲ
-- Mục đích: Xem ai đăng ký môn nào và thuộc học kỳ nào.
-- TRUY VẤN 1: Chi tiết sinh viên đăng ký TẤT CẢ các Học kỳ
-- Mục đích: Xem ai đăng ký môn nào, thuộc học kỳ nào (Kỳ 1, Kỳ 2, hoặc bất kỳ kỳ nào khác).
SELECT
...
-- ĐÃ BỎ MỆNH ĐỀ WHERE LỌC THEO MaHK để hiển thị tất cả các học kỳ
ORDER BY
...
-- Mục đích: Xem kết quả tổng hợp sau khi chức năng (Cursor) chạy.
SELECT
    LHP.MaHK, -- Hiển thị Mã học kỳ
    LHP.MaLHP,
    MH.TenMH AS TenMonHoc,
...
-- Chạy báo cáo cho học kỳ 2 năm 2023-2024
SELECT
    LHP.MaLHP,
    MH.TenMH AS TenMonHoc,
    SV.MaSV,
...
INNER JOIN
    MonHoc AS MH ON LHP.MaMH = MH.MaMH
INNER JOIN
    SinhVien AS SV ON DKHP.MaSV = SV.MaSV -- Nối thêm bảng SinhVien
WHERE 
    LHP.MaHK = N'HK2-2324' -- Lọc theo Mã học kỳ 1
ORDER BY
    LHP.MaLHP, SV.MaSV;
EXEC sp_BaoCaoHocKy @MaHK = N'HK2-2324';
...
* Mục đích: Xử lý logic đăng ký một lớp học phần cho sinh viên.
* Logic:
	1. Nhận vào Mã Sinh viên (@MaSV) và Mã Lớp học phần (@MaLHP).
    2. Kiểm tra xem @MaLHP có tồn tại không. Nếu không, in thông báo và dừng.
    3. Kiểm tra xem @MaSV đã đăng ký @MaLHP này từ trước chưa. Nếu rồi, in thông báo và dừng.
    4. Lấy MaMH (Mã môn học) từ @MaLHP.
    5. Gọi hàm fn_KiemTraTienQuyet(@MaSV, @MaMH) để kiểm tra tiên quyết.
    6. Nếu hàm trả về 0 (Không đủ điều kiện), in thông báo lỗi và dừng.
    7. Nếu vượt qua các kiểm tra, thực hiện INSERT vào bảng DangKyHocPhan.
    8. In thông báo đăng ký thành công.
------------------------------------------------------------
*/
...
BEGIN
    ...
    -- (Trigger trg_KiemTraNgayDangKy và trg_KiemTraSiSo sẽ tự động chạy ở đây)
    INSERT INTO DangKyHocPhan (MaSV, MaLHP)
    VALUES (@MaSV, @MaLHP);
...
select * from LopHocPhan
select *  from MonHoc_TienQuyet
EXEC sp_DangKyMonHoc @MaSV = N'2001140002', @MaLHP = N'LHP04';
GO

/*
------------------------------------------------------------
* Tên trigger: trg_KiemTraNgayDangKy
* Mục đích: Ngăn INSERT nếu ngày đăng ký (ngày hiện tại) nằm ngoài khoảng thời gian cho phép của học kỳ.
* Logic:
    1. Tự động kích hoạt khi có lệnh INSERT mới trên bảng DangKyHocPhan.
    2. Lấy MaLHP từ dòng vừa được chèn.
    3. Tra cứu MaHK (LopHocPhan), sau đó tra cứu NgayBatDau và NgayKetThuc (HocKy).
    4. Kiểm tra xem ngày hiện tại GETDATE() có nằm trong khoảng NgayBatDau và NgayKetThuc hay không.
    5. Nếu ngày hiện tại nằm ngoài khoảng cho phép, báo lỗi và hủy bỏ lệnh INSERT đó.
------------------------------------------------------------
*/
...
BEGIN
    ...
    BEGIN
        -- Nếu tìm thấy vi phạm, báo lỗi và HỦY toàn bộ giao dịch
